import {
  ...
  handleSuccess,
  handleErrors,
} from "@/app/_helpers/web/formatters";
import {
  getCacheData,
  requestHandler,
  gqlF,
...
export async function upload(
  ...
  files.forEach(
    (file: { fileSize: number; name: string; source: string | Blob }) => {
  ...
  if (tooBigFiles) {
    errorMessage(
      `The following files are above ${niceBytes(
        maxSize
      )} and will not be uploaded: ${tooBigFiles}`
    );
  }

  const result = await fetch(
    `/api/upload?ispublic=${isPublic ? "true" : "false"}`,
    {
      method: "POST",
      body: formData,
      credentials: "include",
...
export const deleteFile = async (fileKey: any, isPublic = true) => {
  const request = await requestHandler({
    type: "DELETE",
    route: `upload?key=${fileKey}${isPublic ? "&ispublic=true" : ""}`,
  });
  if (request?.errors) {
...
export const deleteFiles = async (fileKeys: any, isPublic = true) => {
  const request = await requestHandler({
    type: "DELETE",
    route: `upload${isPublic ? "?public=true" : ""}`,
    body: { fileKeys: fileKeys },
  });
  if (request?.errors) {
    errorMessage(request.message);
    throw request;
...
export const makeFilePublic = async (fileKey: any) => {
  const user = await getCacheData("currentUser", true);

  const result = (await fetch(
    process.env.FILE_URL + `make-file-public/${fileKey}?id=${user?.userId}`,
    {
      method: "GET",
      credentials: "include",
    }
...
export const makeFilePrivate = async (fileKey: any) => {
  ...
  const result = (await fetch(
    process.env.FILE_URL + `make-file-private/${fileKey}?id=${user?.userId}`,
    {
      method: "GET",
      credentials: "include",
    }
...
export const sendEmail = async (body: any) => {
  ...
  const result = await fetch(
    process.env.FILE_URL + `send-email?id=${user?.userId}`,
    {
      method: "POST",
