...
import { CurrentEstablishment } from "./hooks/useEstablishment";
...
export let reactRootIds = [] as any[];
...
export const publicId = (currentEstablishment: any) => {
  if (!currentEstablishment?.establishmentName) return null;
  return currentEstablishment.establishmentName.replace(/\s/g, "");
};
...
export const timeTag = (dateTime: string | number | Date | undefined) => {
  ...
  if (dateTime) {
    ...
    output = // eslint-disable-line
      <time>{new Date(dateTime).toUTCString()}</time>;
  }
...
export const parseDataIfString = (value: any) => {
...
export const dateFormat = (date: string, dateType = "date") => {
  if (!date) return null;
...
export const checkRequiredAndErrs = (
  model: any,
  setErrors: (arg0: (errors: any) => any) => any,
  errors: { [x: string]: any }
) => {
