...
export const getSessionKnex = async (connection: Knex): Promise<Session | null> => {
  const sessionCookie = cookies().get("session");
  if (!sessionCookie) return null;

  const rows = await connection("SessionStorage").where({ token: sessionCookie.value }).select("data", "userId").first();

  if (!rows) return null;

  const data = typeof rows.data === "string" ? JSON.parse(rows.data) : rows.data;
  return { ...data, userId: rows.userId } as Session;
};
...
export const SortRecordData = (records: any, orderBy: string, dir: string, existingColumn: boolean) => {
  ...
  if (!Number.isNaN(parseInt(curData[0][orderBy]))) {
    ...
    curData.sort((a: any, b: any) => {
      if (!a[orderBy] || !b[orderBy]) return 0;
      else if (typeof a[orderBy] === "number" && typeof b[orderBy] === "number") return dir === "ASC" ? a[orderBy] - b[orderBy] : b[orderBy] - a[orderBy];

      const aLower = typeof a[orderBy] === "object" ? JSON.stringify(a[orderBy]).toLowerCase() : a[orderBy].toLowerCase();
      const bLower = typeof b[orderBy] === "object" ? JSON.stringify(b[orderBy]).toLowerCase() : b[orderBy].toLowerCase();
      return dir === "ASC" ? aLower.localeCompare(bLower) : bLower.localeCompare(aLower);
    });
  }
