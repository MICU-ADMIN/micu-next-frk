...
export const POST = async (req: Request, res: Response) => {
  try {
    ...
    await (
      await conn
    ).query("INSERT INTO Prayer (title, description,establishmentId, timesData,userId, updatedAt ) VALUES (?, ?, ?, ?, ?, ?)", [
      body.title,
      body.description,
...
export const PUT = async (req: Request, res: Response) => {
  try {
    ...
    await (
      await conn
    ).query("UPDATE Prayer SET title = ?, description = ?,  timesData = ?, updatedAt = ?, WHERE id = ?", [
      body.title,
      body.description,
      body.timesData,
      date,
