...
export const POST = async (req: Request, res: Response) => {
  try {
    ...
    const [SiteQuery] = await (
      ...
      "SELECT id, description, label, permissions, adminIds, userId, thumbnail, createdAt, updatedAt FROM Site WHERE establishmentId = ? AND LOWER(label) LIKE ? ORDER BY id DESC LIMIT 30",
      [session.establishmentId, "%" + body.searchTerm + "%"]
    );

    return NextResponse.json(SiteQuery);
  } catch (err) {
    console.log(err);
