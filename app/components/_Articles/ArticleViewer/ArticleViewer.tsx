...
function ArticleViewer({ article, data }: Props) {
  const { init, destoryEditor } = useEditor();
  const [audioUrl, setAudioUrl] = useArticleStore((state) => [
    state.audioUrl as any,
    state.setAudioUrl,
  ]);
  ...
  const fetchCategories = () => {
    requestHandler({
      ...
      if (res?.data && res?.data?.articleCategories) {
        ...
        res?.data?.articleCategories.forEach(
          (c: { id: string | number; label: any }) => {
  ...
  return (
    <div className="wrap m-10 my-16 flex justify-center align-middle md:px-0">
          ...
          <div className="  mt-3 flex flex-col items-center justify-center">
            ...
            <div className="relative mt-2 flex flex-row items-center justify-center">
              ...
              <div className="absolute right-[-200%] ">
                <span
                  onClick={(e: any) => setSharePopoverEvent(e)}
                  className="cursor-pointer text-[12px] text-gray-500 hover:opacity-70"
                >
                  share
                </span>
                <span
                  onClick={() => setShowComments(true)}
                  className="ml-2 cursor-pointer text-[12px] text-gray-500 hover:opacity-70"
                >
                  comments
                </span>
              </div>
              ...
              {article?.categoryIds &&
                article.categoryIds.map((category: any) => (
                  <span
                    ...
                    className="mb-2 mr-2 cursor-pointer rounded-md bg-primary-500 px-2 py-0.5 text-sm text-white shadow-sm transition-all duration-200 hover:bg-primary-600 hover:shadow-md"
                  >
      ...
      <Popover
        plusY={-20}
        event={sharePopoverEvent}
        show={sharePopoverEvent !== null}
        onClose={() => setSharePopoverEvent(null)}
      >
        <ShareOptions article={article} />
      </Popover>
...
const ShareOptions = ({ article }: any) => {
  return (
    <div className="flex flex-row items-center justify-center">
      ...
      >
        <img
          className="mx-1 h-6 w-6 hover:opacity-50"
      ...
      <a
        href={`mailto:?subject=${article?.title}&body=${window.location.href}`}
        target="_blank"
        rel="noopener noreferrer"
      >
        <img
