...
function ArticleViewer({ article, data, users }: Props) {
  ...
  const [categoryObj, setCategoryObj] = React.useState({}) as any;
  ...
  const fetchCategories = () => {
    requestHandler({
      ...
      if (res?.data && res?.data?.articleCategories) {
        ...
        res?.data?.articleCategories.forEach((c: { id: string | number; label: any }) => {
  ...
  return (
    <div className="wrap m-10 my-16 flex justify-center align-middle md:px-0">
      ...
      <div style={{ width: "1200px" }} className="flex flex-row flex-wrap ">
        <div className="grid grid-cols-1 " style={{ width: data?.hideSideBar ? "100%" : "67%" }}>
          {/* Featured Image */}
          {!data?.hideFeatureImage && (
            <img
        ...
        object-cover object-center"
          ...
          <div className="  mt-3 flex flex-col items-center justify-center">
            {!data?.hideTitle && (
              <h2
                style={{
                  color: data?.titleColor || "initial",
                  fontSize: data?.titleSize ? data?.titleSize + "px" : "32px",
                  fontWeight: data?.titleWeight || "600",
                }}
            ...
            <div className="relative mt-2 flex flex-row items-center justify-center">
              {!data?.hideMetaData && (
                <>
                  <span style={{ color: data?.accentColor || "rgb(107 114 128)" }} className="text-sm ">
                    {article?.createdAt ? new Date(article?.createdAt).toDateString() : ""}
                  </span>
                  <span className="mx-2 text-sm text-gray-500">|</span>
                  <span style={{ color: data?.accentColor || "rgb(107 114 128)" }} className="text-sm text-gray-500">
                    By {users[article?.userId]?.firstName || "Unknown"}
                  </span>
                </>
              )}

              <div className="absolute right-[-160%] " style={{ color: data?.accentColor || "rgb(107 114 128)" }}>
                {!data?.hideShare && (
                  <span onClick={(e: any) => setSharePopoverEvent(e)} className="cursor-pointer text-[12px] text-gray-500 hover:opacity-70">
                    share
                  </span>
                )}
                {!data?.hideComment && (
            ...
            {!data?.hideDivider && <hr style={{ borderColor: data?.dividerColor || "rgb(209 213 219)" }} className="my-4 w-full" />}
          ...
          <div className="grid grid-cols-1 gap-4" style={{ width: "32%" }}>
            ...
            <div className="ml-14">
              ...
              <div className=" flex flex-wrap">
                {article?.categoryIds &&
                  article.categoryIds.map((category: any) => (
                    <span
                      key={category}
                      style={{
...
const ShareOptions = ({ article }: any) => {
  return (
