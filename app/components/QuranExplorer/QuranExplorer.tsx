...
import { getCacheData, addToCache } from "@/app/_helpers/web/requestHandler";
...
type Props = {
  close: () => void;
  addVerse: (verse: { surah: number; ayah: number }, text: string) => void;
  presetIndex?: { surah: number; ayah: number };
  readOnly?: boolean;
};

function QuranExplorer({
  close,
  addVerse,
  presetIndex,
  readOnly = false,
  ...
  const [selectedTafsir, setSelectedTafsir] = React.useState(
  ...
  const fetchAyah = async () => {
    const cached = await getCacheData(
      `tafsir/${selectedSurah}:${selectedAyah}:${selectedTafsir}`
    );
    if (cached) {
  ...
  const fetchAdditionalTafsir = async (tafsir) => {
    console.log(tafsir);
    const cached = await getCacheData(
      `tafsir/${selectedSurah}:${selectedAyah}:${tafsir}`
    );
    if (cached) {
  ...
  const fetchVerses = async () => {
    const cached = await getCacheData(`verses/${curPage}/${translationKey}`);
    if (cached) {
  ...
  return (
    <>
      <div
        style={{ width: "90vw" }}
        className="modal-center modal-center-large  h-[94vh] p-8 "
      >
        <div className="flex flex-col">
          <div className="flex flex-row items-center justify-between">
            <div className="flex flex-row items-center">
              ...
              />
              <select
                className="base-input ml-3"
                value={selectedAyah}
                onChange={(e) => setSelectedAyah(e.target.value)}
              >
                {Array(
                  quranExplorerData.surahOptions[selectedSurah - 1]?.verses
                )
                  .fill()
                  .map((_, index) => (
                ...
                <Button
                  ...
                  onClick={() => {
                    if (selectedAyah == 1) {
                      ...
                      setTimeout(() => {
                        setSelectedAyah(
                          quranExplorerData.surahOptions[selectedSurah - 1]
                            ?.verses || 1
                        );
                      }, 0);
                    } else {
              ...
              <div className="" key={ayah?.id}>
                <div className="hide-scrollbar mt-2 flex max-h-[25vh] flex-row-reverse flex-wrap overflow-y-scroll p-0.5">
                  {ayah?.verses &&
                    Object.keys(ayah?.verses).map((key) => {
                      ...
                      const last: any =
                        verse?.words?.[verse?.words?.length - 1];
                      if (
                        last.page_number !== curPage &&
                        last.page_number !== lastPage
                      ) {
                ...
                >
                  <p
                    ...
                    style={{
                      fontSize:
                        selectedTafsir && selectedTafsir.startsWith("en")
                          ? "1rem"
                          : "1.25rem",
                    }}
                    ...
                    <div className="mt-5 " key={a?.resource_name}>
                      <div className="ml-1 flex flex-row items-center">
                        ...
                        <Button
                          ...
                          className="m-2"
                          onClick={() =>
                            setAdditionalTafsir((prev) =>
                              prev.filter(
                                (t) => t.resource_name !== a.resource_name
                              )
                            )
                          }
                        >
          ...
          <div
            className="  hide-scrollbar flex flex-row  flex-wrap overflow-y-scroll"
            style={{ resize: "vertical", direction: "rtl" }}
          >
            {pageVerses?.map((verses) =>
              verses?.words?.map((word) => (
                <QuranWord
                  key={word?.id}
                  word={word}
                  text
            ...
            <ClipboardIcon
              onClick={() =>
                copyToClipboard(
                  pageVerses.map((v) => v.text_uthmani)?.join(" "),
          ...
          <hr className="my-4" />
          <div
            className=" flex flex-row  flex-wrap overflow-y-scroll"
            style={{ resize: "vertical" }}
          >
            {pageVerses?.map((verses) =>
              verses.translations?.map((word) => (
                <p
                  key={word?.id}
                  className="m-[2px] text-[16px]  text-gray-800"
      ...
      <Popover
        plusY={-20}
        event={tafsirsPopoverEvent}
        show={tafsirsPopoverEvent !== null}
        onClose={() => setTafsirsPopoverEvent(null)}
        placement={undefined}
      >
        <select
          className="base-input"
          onChange={(e) => fetchAdditionalTafsir(e.target.value)}
        >
