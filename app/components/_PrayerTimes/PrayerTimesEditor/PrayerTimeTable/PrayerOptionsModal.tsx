...
import { Prayer } from "@/_types/dbTypes";
import Popover from "@/app/components/Modals/Popover/Popover";

type Props = {
  setShowPrayerTimesEditor: any;
  prayerTimes: Prayer[];
  setPrayerTimes: any;
};

function PrayerOptionsModal({
  setShowPrayerTimesEditor,
  prayerTimes,
  setPrayerTimes,
}: Props) {
  ...
  const [importMenuEvent, setImportMenuEvent] = React.useState(null) as any;
  const [importOptions, setImportOptions] = React.useState({
    data: null,
    headers: null,
  });
  const formatCsv = (csv: Blob) => {
  ...
  React.useEffect(() => {
    ...
    const columns = prayerFields.map((col) => ({
      name: col.name,
      selector: (row: { [x: string]: any }) => row[col.key],
      sortable: true,
      width: "120px",
      cell: (
        row: unknown,
        index: unknown,
        column: any //@ts-ignore
      ) =>
        React.useCallback(
          <Cell
            row={row}
          ...
          {!showImportOpts ? (
            <div>
              <div className="flex justify-between">
                <div className="m-1 flex flex-row">
                  ...
                  <div className="ml-4">
                    <input
                      ...
                      onChange={(e) =>
                        setDataFilter(e.target.value.toLowerCase())
                      }
                    />
                  </div>
                </div>
                <div>
                  <button
                    className="focus:shadow-outline mr-1 rounded bg-primary-500 px-2 py-1  text-white hover:bg-primary-400 focus:outline-none"
                  ...
                  <button
                    className="focus:shadow-outline rounded bg-primary-500 px-2 py-1  text-white hover:bg-primary-400 focus:outline-none"
                    onClick={onSave}
                  >
                    Save
                  </button>
              ...
              <DataTable
                columns={columns}
                data={prayerTimes.filter((data) =>
                  data.date.toLowerCase().includes(dataFilter)
                )}
                pagination
            ...
            <TimesImporter
              ...
              setTableData={setPrayerTimes}
              close={() => setShowImportOpts(false)}
            />
          )}
        </div>
      </div>
      <div
        onClick={() => setShowPrayerTimesEditor(false)}
        className="modal-overlay"
      ></div>

      <Popover
        event={importMenuEvent}
        onClose={() => setImportMenuEvent(null)}
        show={importMenuEvent !== null}
      >
        <div className="m-1 flex h-[100px] w-[300px] flex-col ">
          <label className="mb-1 mt-1">Import Prayer Times CSV</label>
          <input
            type="file"
            className="mr-2 block w-full rounded-md border-gray-300 p-2 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"
            accept=".csv"
      ...
      onBlur={(e) => {
        ...
        setTableData((prev: any[]) => {
          const findIndex = prev.findIndex(
            (data: { id: any }) => data.id === row.id
          );
          prev[findIndex][prop] = e.target.value;
          return prev;
        });
      }}
      className="w-full rounded-md border border-gray-300 p-2 transition duration-150 ease-in-out focus:border-primary-500 focus:outline-none sm:text-sm sm:leading-5"
    />
  );
};
