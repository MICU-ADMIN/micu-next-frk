...
function PrayerTimeTable({
  ...
  const [showPrayerTimesEditor, setShowPrayerTimesEditor] =
    ...
    <>
      {showPrayerTimesEditor && (
        <PrayerOptionsModal
          ...
          setPrayerTimes={editOptions.setPrayerTimes}
          setShowPrayerTimesEditor={setShowPrayerTimesEditor}
        />
      )}
      {editOptions?.setPrayerTimes && (
        <PencilSquareIcon
          className="fixed right-2 z-50 h-5 w-5 cursor-pointer text-primary-500 hover:text-gray-700 "
          aria-hidden="true"
        ...
        >
          ...
          <tbody>
            <tr
              style={{
                backgroundColor:
                  nextPrayer.name === "Fajr" ? theme.highlightColor + "80" : "",
            ...
            <tr
              style={{
                backgroundColor:
                  nextPrayer.name === "Asr" ? theme.highlightColor : "",
            ...
            <tr
              style={{
                backgroundColor:
                  nextPrayer.name === "Isha" ? theme.highlightColor + "80" : "",
...
type PrayerTimeProps = {
  time: { name: string; time: string; jamaat: string };
  nextPrayer: { name: string };
  theme: any;
  currentTime: any;
};

function PrayerTIme({ time, nextPrayer, theme, currentTime }: PrayerTimeProps) {
  ...
  const calculateTimeLeft = () => {
    ...
    const eventTimeStamp = new Date(
      `${new Date().toDateString()} ${time.time}`
    ).getTime();
  ...
  return (
    <article
      aria-labelledby={`episode-${time.name}-title`}
      ...
      <Container className="flex flex-row justify-between">
        <div className="flex flex-col items-start ">
          ...
          <p
            ...
            style={{ fontSize: "1.5rem", color: theme.textColor }}
          >
          ...
          {nextPrayer.name == timeData.name && (
            <p className="absolute right-10 ">
              <span
                style={{ fontWeight: "600", color: theme.highlightColor }}
                className="text-3xl"
              >
