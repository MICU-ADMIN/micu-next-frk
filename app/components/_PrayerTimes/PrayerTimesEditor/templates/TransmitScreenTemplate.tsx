...
import Editor from "@/app/components/Elements/Editor/Editor";
...
function AboutSection({ subMessage, ...props }) {
  let [isExpanded, setIsExpanded] = useState(false);

  return (
    <section {...props}>
      <h2
        style={{ color: props?.theme?.timeCardTextColor }}
        className="flex items-center font-mono text-sm font-medium leading-7"
      >
        <TinyWaveFormIcon
      ...
      ></p>
      {props.editOptions?.setSubMessage && (
        <PencilSquareIcon
          className="ml-2 mr-2 h-5 w-5 cursor-pointer text-primary-500 hover:text-gray-700"
          aria-hidden="true"
          title='Edit "Sub Message"'
          onClick={() => props.setShowSubMessageEditor(true)}
        />
      )}

      {props.showSubMessageEditor && (
        <div
          className="absolute left-0 top-[350px] z-50 m-4 h-fit  w-[95%] rounded-lg bg-white shadow-lg"
        ...
        >
          <Editor
            ...
            value={subMessage}
            onChange={(content) => {
              props.editOptions.setSubMessage(content);
            }}
            onCancel={() => props.showSubMessageEditor(false)}
          />
        </div>
      )}
      {/* {!isExpanded && (
...
export default function TransmitScreenTemplate({
  theme,
  ...
  let hosts = [
    ...
    currentEstablishment?.zip || "",
  ];

  let HijriDate = null;

  try {
    HijriDate = toHijri(new Date(prayerTime?.date));
  } catch (e) {
    console.log(e);
  }
  ...
  return (
    <div
      className="h-full"
      style={{ backgroundColor: theme.backgroundColor, overflow: "hidden" }}
      onClick={() => {
    ...
    >
      <header
        className=" overflow-y-hidden lg:fixed lg:inset-y-0 lg:left-0 lg:flex lg:items-start"
        style={{
          zIndex: 99,
          background: theme.timeCardColor,
          overflow: "hidden",
          maxWidth: "35vw",
        }}
      >
        <div className="hidden lg:sticky lg:top-0 lg:flex lg:w-16 lg:flex-none lg:items-center lg:whitespace-nowrap lg:py-12 lg:text-sm lg:leading-7 lg:[writing-mode:vertical-rl]">
          <span style={{ color: theme?.highlightColor }} className="font-mono">
            MosqueICU®
          </span>
          <span
            className="mt-6 flex gap-6 font-bold "
        ...
        <div className="relative z-10 mx-auto px-4 pb-4 pt-10 sm:px-6 md:max-w-2xl md:px-4 lg:min-h-full lg:flex-auto lg:border-x lg:border-slate-200 lg:px-8 lg:py-12 xl:px-12">
          ...
          <div className="mt-10 text-center lg:mt-12 lg:text-left">
            ...
            <p
              style={{ color: theme.timeCardTextColor }}
              className="mt-3 text-lg font-medium leading-8 text-slate-700"
              dangerouslySetInnerHTML={{ __html: topMessage }}
            ></p>
          ...
          {showTopMessageEditor && (
            <div
              className="top-[20 0px] absolute  left-0 z-50 m-4 h-fit  w-[95%] rounded-lg bg-white shadow-lg"
              onClick={(e) => e.stopPropagation()}
            >
              <Editor
                title="Edit Top Message"
          ...
          <AboutSection
            ...
            showSubMessageEditor={showSubMessageEditor}
            setShowSubMessageEditor={setShowSubMessageEditor}
            theme={theme}
          ...
          <section className="mt-10 lg:mt-12">
            <h2 className="sr-only flex items-center font-mono text-sm font-medium leading-7 text-slate-900 lg:not-sr-only">
              <TinyWaveFormIcon
                colors={["fill-primary-300", "fill-primary-300"]}
                className="h-2.5 w-2.5"
              />
              <span style={{ color: theme?.textColor }} className="ml-2.5">
                Our Feed
              </span>
            </h2>
      ...
      <main className="lg:ml-112 xl:ml-120 border-t border-slate-200 lg:relative lg:mb-28 lg:border-t-0">
        ...
        <div className="relative left-[40%] w-[55%]">
          <div className="relative top-2">
            <div
              className="m-3 rounded-lg p-1 shadow-lg"
              style={{
            ...
            >
              ...
              <p
                className="f mb-4 text-center"
                style={{ color: theme.timeCardTextColor, fontSize: "1.5vw" }}
              >
