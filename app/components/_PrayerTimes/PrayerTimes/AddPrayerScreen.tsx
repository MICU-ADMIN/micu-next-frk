...
type Props = {
  close: () => void;
  setCurScreens: (prev: any) => void;
  establishmentPublicId: string;
};

function AddPrayerScreen({
  close,
  setCurScreens,
  establishmentPublicId,
}: Props) {
    ...
    requestHandler({ type: "post", route: "prayerScreen", body: data })
      .then((res) => {
        ...
        if (res.success) {
          ...
          setCurScreens((prev: any) => [...prev, curData]);
    ...
    <>
      <div className="modal-center">
        <div className="modal-center-medium h-[95vh] overflow-hidden rounded-md bg-white p-8 shadow-xl">
          <h2 className="text-primary-600 mx-1">Add Prayer Screen</h2>

          <p className="mt-6 text-gray-500 mx-1">
            Provide a title and description for your prayer screen. You will be
            able to additionally customise the screen and add prayers times once
