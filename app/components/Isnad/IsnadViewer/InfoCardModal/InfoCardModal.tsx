            ...
            >
              <Dialog.Panel className="w-full max-w-xl transform overflow-hidden ring-tremor bg-white p-6 text-left align-middle shadow-tremor transition-all rounded-xl">
                <div className="flex flex-row justify-between">
                  <p className="text-base text-primary-500 font-medium">
                    {scholarData?.designation || "Details"}
                  </p>
                ...
                <div className="relative mt-4 h-[700px] overflow-y-scroll">
                  <p>
                    {scholarData?.bornAt || data?.bornAt} -{" "}
                    {scholarData?.diedAt || data?.diedAt} (Age)
                  </p>
...
const parseScholarHtml = (
  ...
  const rows = Array.from(table?.children[0].children).map((row: any) =>
    Array.from(row.children).map((cell: any) => cell.innerText)
  );
  // console.log(rows);
