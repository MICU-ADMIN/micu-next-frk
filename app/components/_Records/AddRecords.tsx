...
const Form = dynamic(() => import("@/app/components/Elements/Form/Form"), {
  loading: () => <Spinner />,
});
    ...
    component({ model, field, fields }) {
      return (
        <img
          src={model.thumbnail}
          alt="thumbnail"
          className=" mb-5 mt-4  h-[30vh] w-full rounded-md border border-gray-300 object-cover p-1 transition duration-150 ease-in-out focus:border-primary-500 focus:outline-none sm:text-sm sm:leading-5"
        />
      );
    },
  },
...
function AddRecords({ close, setCurRecords, publicEstablishmentId }: Props) {
  ...
  const onSubmit = async (data: {
    ...
    curData = data;
    requestHandler({ type: "post", route: "records", body: data }).then(
      (res) => {
        setLoading(false);
    ...
    <>
      <div className="modal-center">
        <div className="modal-center-medium h-[95vh] overflow-hidden rounded-md bg-white p-8 shadow-xl">
          <h2 className="mx-1 text-primary-600">Add Reord</h2>

          <p className=" mx-1 mt-6 text-gray-500">
            Provide a title and description for Record
          </p>
