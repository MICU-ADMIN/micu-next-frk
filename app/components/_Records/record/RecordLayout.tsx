...
function RecordLayout({ record, properties, navOpen, setLoading, loading, setRecord, publicEstablishmentId, view }: Props) {
  ...
  let filterTimeout = React.useRef<any>(null);
      ...
      <div className="m-5 mt-1">
        <TableElement
          ...
          fetchHandler={(lastId: number, cb: (arg0: any) => void, sorts?: any, filters?: any) => {
            !filters
              ? fetchRecords(lastId, record.id, setFetching, fetching, cb, sorts)
              : (filterTimeout.current && clearTimeout(filterTimeout.current),
                (filterTimeout.current = setTimeout(() => {
                  fetchFilteredRecords(lastId, record.id, setFetching, fetching, cb, sorts, [...filters]);
                }, 350)));
          }}
