...
function ElementWrapper({
  ...
  React.useLayoutEffect(() => {
    if (element?.size && targetRef.current) {
      ...
      targetRef.current.style.width = widthpx && widthpx < 300 ? "300px" : element?.size.width || "175px";
  ...
  const onTransform = React.useCallback(
    (e: { target: { style: { transform: string; left: string; top: string } }; transform: any; left: number; top: number }) => {
    ...
    <>
      {(isFocused || isHovering) && !readOnly && (
        <>
          ...
          >
            <Edit
              className="h-[16px]  min-w-[16px]  cursor-pointer hover:text-indigo-600 transition-all duration-300"
              onClick={(e) => {
                e.stopPropagation();
        ...
        {isFocused && !readOnly && !showEditMenu /* @ts-ignore */ && (
          <Moveable
            ...
            onResize={(e) => {
              ...
              setSize({ width: width + "%", height: element.type == "text" ? "fit-content" : e.height + "px" });
            }}
          />
        )}
      </Suspense>
