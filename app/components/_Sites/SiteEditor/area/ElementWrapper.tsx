...
type Props = {
  ...
  elementIndex: number;
  onChange: () => void;
  duplicateElement: (id: number, e: any) => void;
...
function ElementWrapper({
  ...
  React.useLayoutEffect(() => {
    if (element?.size && targetRef.current) {
      //get the width percentage is px

      let widthpx: any;
      // if (element.type === "text" && element?.size.width) {
      //   const width = element?.size.width.replace("%", "");
      //   widthpx = window.innerWidth * (width / 100);
      // }

      targetRef.current.style.width =
        widthpx && widthpx < 300 ? "300px" : element?.size.width || "175px";
      targetRef.current.style.height = element?.size.height
        ? element?.size.height
        : "175px";
  ...
  const formatGuideLines = () => {
    ...
    for (
      let i = 0;
      i < wrapperRef.current?.offsetWidth;
      i += wrapperRef.current?.offsetWidth / 100
    ) {
  ...
  const onTransform = React.useCallback(
    (e: {
      target: { style: { transform: string; left: string; top: string } };
      transform: any;
      left: number;
      top: number;
    }) => {
  ...
  return (
    <>
      {(isFocused || isHovering) && !readOnly && (
        <>
          <div
            className="card flex flex-row items-center rounded-md bg-white p-2 shadow-lg border border-gray-500/50 bg-opacity-95 backdrop-blur-10 shadow-blur-10 shadow-2xl border-2 border-gray-300"
            style={{
              position: "absolute",
          ...
          >
            <Edit
              className="h-[16px]  min-w-[16px]  cursor-pointer hover:text-primary-600"
              onClick={(e) => {
                e.stopPropagation();
                setShowEditMenu(
                  showEditMenu === element.id ? null : element.id
                );
              }}
            />
            <Copy
              className="h-[16px]  min-w-[16px]  cursor-pointer hover:text-primary-600 "
              onClick={(e) => duplicateElement(element.id, e)}
            />
            {!onDelete ? (
              <Trash
                onClick={(e) => {
            ...
            <span className="  ml-2 rounded bg-primary-500  px-1 py-[1.5px] text-xs font-semibold uppercase text-white">
              {element?.type}
            </span>
          </div>
        </>
      ...
      <div
        ...
        ${
          !readOnly
            ? "cursor-pointer hover:border-gray-300 hover:shadow-lg hover:outline-none hover:ring-2 hover:ring-primary-500 focus:border-gray-300 focus:shadow-lg  focus:outline-none focus:ring-2 focus:ring-primary-500"
            : ""
        }`}
        style={{
          zIndex:
            !readOnly && showEditMenu === element.id
              ? 999999
        ...
        {isFocused && !readOnly && !showEditMenu /* @ts-ignore */ && (
          <Moveable
            ...
            onResize={(e) => {
              //convert pixels to percentage
              const width = (
                (e.width / wrapperRef.current?.offsetWidth) *
                100
              ).toFixed(2);
              const height = (
                (e.height / wrapperRef.current?.offsetHeight) *
                100
              ).toFixed(2);
              ...
              setSize({
                width: width + "%",
                height:
                  element.type == "text" ? "fit-content" : e.height + "px",
              });
            }}
          />
        )}
