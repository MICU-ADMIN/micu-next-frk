...
function ElementMenuWrapper({
  ...
  const targetLeft =
    targetRef?.current?.offsetLeft + targetRef?.current?.offsetWidth + 10;
    ...
    <>
      ...
      >
        <div
          ...
          className=" slideLeft relative  items-center   rounded-md border border-gray-200 bg-white p-2 drop-shadow-lg hide-scroll hide-scrollbar backdrop-blur-50 shadow-blur-10 shadow-2xl border-2 border-gray-300"
        >
          <XIcon
            ...
            {options.map((option: any, i: number) => {
              return (
                <div
                  ...
                  ${
                    renderedIndex === i
                      ? "text-primary-500 font-semibold border-b-1 border-primary-500 border-primary-500 transition-all duration-200"
                      : ""
                  }`}
                  onClick={() => {
                    setRenderedIndex(i);
          ...
          <hr />
          <div className="flex-column mt-2 flex w-full min-w-[255px] p-2 slideLeft">
            {options[renderedIndex].component}
          </div>{" "}
          {submit && (
            <Button
