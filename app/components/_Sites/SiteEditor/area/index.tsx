  ...
  return (
    <div
      className={` relative max-w-[100%]  ${
        !readOnly && !isSlide
          ? "input-wrapper-resizer hover:border-transparent hover:shadow-lg hover:outline-none hover:ring-2 hover:ring-primary-600  focus:shadow-lg  focus:outline-none focus:ring-2 focus:ring-primary-600 "
          : ""
      }  `}
      style={{
        height: !under1100
    ...
    >
      {hovering && !readOnly && !isSlide && (
        <>
          <div
            style={{ zIndex: 99999 }}
            className="card  absolute right-[2px] rounded-lg p-2 opacity-90 shadow-lg slideLeft  bg-[rgba(255, 255, 255, 0.658)]"
          >
            <Button
              variant="secondary"
              size="smd"
            ...
            <Button
              style={{ backgroundColor: "#fe1818b3" }}
              size="smd"
            ...
            <div className="flex flex-row  mt-2 z-9999">
              <ArrowDownCircleIcon
                className={`text-gray-500 h-6 w-6 cursor-pointer hover:text-primary-500  ${
                  lastItem ? "opacity-30" : ""
                } `}
              ...
              />
              <ArrowUpCircleIcon
                className={`ml-2 text-gray-500 h-6 w-6 cursor-pointer hover:text-primary-500  ${
                  index == 0 ? "opacity-30" : ""
                } `}
          ...
          {isHovering && (
            <div
              className="absolute h-full w-[2px]  opacity-70 top-0 left-1/2 transform -translate-x-1/2 z-10 border-l border-primary-500 border-dashed
            "
            ></div>
          )}
        </>
      ...
      <section
        style={{
          background:
            data?.backgroundType === "color"
              ? data?.backgroundColor
                ? data?.backgroundColor
                : siteData?.siteColors[
                    siteData?.defaultColorIndexes?.sectionBackground
                  ]
              : "none",
          ...
          backgroundImage:
            data?.backgroundType === "link"
              ? `url(${data?.backgroundImage || ""})`
              : data.backgroundType === "image"
              ? `url(${data?.imageUrl || ""})`
              : "",
          boxShadow: data?.boxShadow
            ? ` 0 0 0 ${data?.boxShadowWidth || "5"}px ${data?.boxShadow || ""}`
            : "",
          margin:
            data?.margin === "full"
              ? data.marginFull + "px"
              : `${data?.marginTop || 0}px ${data?.marginRight || 0}px ${
                  data?.marginBottom || 0
                }px ${data?.marginLeft || 0}px`,
          padding:
            data.padding === "full"
              ? data.paddingFull + "px"
              : data.padding === "side"
              ? `
          ${data?.padding?.top || 0}px ${data?.padding?.right || 0}px ${
                  data?.padding?.bottom || 0
                }px ${data?.padding?.left || 0}px`
              : "",
          border: data?.borderType
            ? `${data?.borderWidth}px ${data?.borderType} ${
                data?.borderColor || "black"
              }`
            : "none",
          animation: data?.animation
            ? `${data?.animation} ${data?.animationDuration || 1}s 
            ${data?.animationDelay || 0}s ${
                data?.animationIterationCount || 1
              } ${data?.animationDirection || "normal"} ${
                data?.animationFillMode || "none"
              } ${data?.animationPlayState || "running"}`
            : "",
        }}
        className={` area-wrapper  h-full  
        ${
          data?.backgroundParallax === "fixed"
            ? "bg-fixed"
            : data?.backgroundParallax === "Animate"
            ? "bg-local"
            : ""
        }  `}
      >
        {data?.backgroundOverlay && (
          <div
            style={{
              boxShadow: `${
                data?.backgroundOverlay
                  ? "inset 0 0 0 1000px " + data?.backgroundOverlay + "50"
                  : ""
              }`,
            }}
        ...
        {elements.length > 0 ? (
          elements.map(
            (element: { id: React.Key | null | undefined }, i: number) => (
              <ElementWrapper
                key={element.id}
                siteData={siteData}
                wrapperRef={ref}
