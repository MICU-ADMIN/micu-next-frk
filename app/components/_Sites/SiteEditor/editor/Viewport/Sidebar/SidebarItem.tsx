...
export const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  return (
    <SidebarItemDiv
      visible={visible}
      height={height}
      className="flex flex-col rounded shadow-sm mb-1 border border-gray-100  hover:border-primary-200"
    >
      <HeaderDiv
        onClick={() => {
          if (onChange) onChange(!visible);
        }}
        className={`flex cursor-pointer items-center rounded bg-white px-2 last:border-b-0 ${
          visible ? "shadow-sm" : ""
        }`}
      >
      ...
      {visible ? (
        <div className="w-full flex-1 overflow-auto slideInDown">
          {children}
        </div>
      ) : null}
    </SidebarItemDiv>
