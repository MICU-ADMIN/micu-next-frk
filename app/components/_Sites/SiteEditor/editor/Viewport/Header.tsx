...
export const Header = ({ pages, save, preview, readOnly, setNewPage, editStates = [], currentPage, undo, setShowPageSidebar }: Props) => {
      ...
      <HeaderDiv className="header w-full text-white transition">
        ...
        >
          ...
          <div title="Page settings">
            <Settings
              className="ml-5 h-[20px] cursor-pointer rounded text-gray-600 transition hover:bg-indigo-300 hover:text-indigo-500 "
              onClick={() => setShowSettingsMenu(true)}
            />
          </div>
          <div title="Add Page">
            <PlusCircle
              className="ml-2 h-[20px] cursor-pointer rounded text-indigo-500 transition hover:bg-indigo-300 hover:text-indigo-500 "
              onClick={() => {
                preview();
