...
const notPlaceholderTypes = ["button", "file", "id", "createdAt", "updatedAt", "select", "checkbox", "number", "date", "users", "color", "userId"];
...
export const Refetch = (fetchHandler: any, setModel: React.Dispatch<React.SetStateAction<any[]>>, requestTransformer?: (arg0: any) => any) => {
  if (fetchHandler) {
...
export const HandleTableFilter = (
  filters: any[],
  setModel: any,
  fetchHandler?: (arg0: any, arg1: (arg0: any) => void | undefined, arg2?: any, arg3?: any) => void,
  requestTransformer?: (arg0: any) => any,
  setNoMoreData?: React.Dispatch<React.SetStateAction<boolean>>
) => {
  if (!fetchHandler) return;

  let sorts = null as any;
  const curFilters = [] as any;

  filters.forEach((f) => {
    if (f.sortFilter) sorts = f;
    else curFilters.push(f);
  });
  ...
  fetchHandler(
    ...
    sorts || null,
    curFilters.length !== 0 ? curFilters : null
  );
};
