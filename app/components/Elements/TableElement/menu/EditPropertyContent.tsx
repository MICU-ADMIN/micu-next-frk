...
function EditPropertyContent({ property, index, onDeleteProperty, currentProperty, setCurrentProperty, changes, setChanges }: Props) {
  ...
  return (
    <div className="flex flex-col w-full mt-3">
      <FormGroup
        ...
        fields={[
          ...
          },
          {
            ...
            title: "Required",
            type: "checkbox",
            hidden: ({ model }: any) => model.type !== "input" && model.type !== "select" && model.type !== "editor" && model.type !== "number",
          },
          {
            name: "max",
            placeholder: "Max",
          ...
          },
          {
            ...
            component: ({ model }: any) => (
              <div className="flex flex-col my-2">
                {currentProperty.options && currentProperty.options.length > 0 ? (
                  <div className="flex flex-row flex-wrap mt-2">
                    {currentProperty.options.map((option: any, index: number) => (
                      <Badge
                        onClick={() => {
                          setCurOption(option);
                          console.log(index);
                  ...
                  <span className="text-xs mt-1 ml-3 text-gray-400">No options added yet</span>
                )}
                <PlusCircle
                  onClick={() => {
                  ...
                  className="h-5 w-5 text-indigo-500 cursor-pointer mt-3 ml-2"
                />

                {curOption && (
                  <div className="flex flex-col mt-3 fadeIn">
                    <p className="text-indigo-500">{showOptionAdd ? "Add option" : "Edit option"}</p>
                    <FormGroup
                      className="w-full"
                      altSetModel={true}
                      errors={errors}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (showOptionAdd) {
                            setCurrentProperty({
                              ...currentProperty,
                              options: [...currentProperty.options, { label: curOption.value, value: curOption.value, id: Date.now() }],
                            });
                          } else {
                            setCurrentProperty((prev: any) => {
                              const options = prev.options.map((option: any) => {
      ...
      {property.name !== "value" ? (
        !onDelete ? (
          <Trash2 className="h-5 w-5 text-red-500 cursor-pointer" onClick={() => setOnDelete(true)} />
        ) : (
          <div className="flex flex-col items-center fadeIn">
            <p className="text-red-500 mr-2">Confirm delete?</p>
            <div className="flex flex-row mt-2 ">
              <Button size="sm" onClick={() => onDeleteProperty(index)} style={{ background: "#D54040" }} className="mr-2">
                Delete
              </Button>
              <Button size="sm" onClick={() => setOnDelete(false)} variant="secondary">
                Cancel
              </Button>
            </div>
          </div>
        ...
        <span className="text-xs text-gray-400">This property cannot be deleted</span>
      )}
    </div>
  );
}
