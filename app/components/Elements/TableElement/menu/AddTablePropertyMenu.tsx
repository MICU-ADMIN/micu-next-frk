...
function AddTablePropertyMenu({ close, onAddProperty, wrapperRef }: Props) {
  ...
  return (
    <>
      ...
      >
        <p className="text-primary-500">Add new property</p>
        ...
        <div className="flex flex-col w-full mt-3">
          ...
          {properties.map((property, index) => {
            return (
              <div
                key={index}
                className="flex flex-col  py-1 px-1 cursor-pointer hover:bg-gray-100 rounded  shadow-sm"
              >
                <div
                  className="flex flex-row items-center"
                  onClick={() => onAddProperty(property.value)}
                >
                  ...
                  {property.value === "input" && (
                    ...
                    >
                      - View options{" "}
                      <ArrowSmallDownIcon className="w-4 h-4 ml-1" />
                    </span>
                  )}
                  {property.value === "date" && (
                    <span
                    ...
                    {inputTypes.map((inputType, index) => {
                      return (
                        <p
                          ...
                          onClick={() =>
                            onAddProperty(property.value, inputType.value)
                          }
                        >
                          {inputType.label}
                        </p>
