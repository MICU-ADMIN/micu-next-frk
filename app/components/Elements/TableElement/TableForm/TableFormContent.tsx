...
const FormSlides = dynamic(() => import("./FormSlides"), { loading: () => <Spinner /> });
const FormArticles = dynamic(() => import("./FormArticles"), { loading: () => <Spinner /> });
const TableFiles = dynamic(() => import("./TableFiles"), { loading: () => <Spinner /> });

const tabs = ["Slides", "Articles", "Uploads"];

function TableFormContent({ row, onValChange, columns, field, publicEstablishmentId, users, setOpenFile }: any) {
  ...
  const checkErrors = (v: any, column: { required: any; maxLength: number; minLength: number; subType: string; name: string }) => {
    if (column.required && !v) {
      setErrors((errors: any) => ({ ...errors, [column.name]: "This field is required" }));
      return "This field is required";
    }

    if (!v) {
      if (errors[column.name]) setErrors((errors: any) => ({ ...errors, [column.name]: "" }));
      return null;
    }

    if (column.maxLength && v.length > column.maxLength) {
      setErrors((errors: any) => ({ ...errors, [column.name]: `Max length is ${column.maxLength}` }));
      return `Max length is ${column.maxLength}`;
    } else if (column.minLength && v.length < column.minLength) {
      setErrors((errors: any) => ({ ...errors, [column.name]: `Min length is ${column.minLength}` }));
      return `Min length is ${column.minLength}`;
    }

    if (column.subType === "email" && emailRegex.test(v) === false) {
      setErrors((errors: any) => ({ ...errors, [column.name]: "Invalid email" }));
      return "Invalid email";
    } else if (column.subType === "phone" && phoneRegex.test(v) === false) {
      setErrors((errors: any) => ({ ...errors, [column.name]: "Invalid phone number" }));
      ...
      setErrors((errors: any) => ({
        ...errors,
        [column.name]: "Password must contain at least 8 characters, one uppercase, one lowercase, one number and one special case character",
      }));
      ...
      setErrors((errors: any) => ({ ...errors, [column.name]: "Only characters are allowed" }));
  ...
  return (
    <div className=" p-8">
      <Editor
        theme="bubble"
        className="text-indigo-500 text-2xl mt-1 outline-none border-none"
        value={row.value}
        disableFocus
        onChange={(v: string) => {
        ...
        {columns.map((column: any, index: number) => {
          ...
          return (
            <div
              ...
              className={`flex flex-row w-full h-fit  border-l border-r border-gray-100  p-2  ${index == 1 ? "rounded-tl-md rounded-tr-md border-t" : ""}
                ${index == columns.length - 2 ? "rounded-bl-md rounded-br-md border-b" : ""}
            `}
            >
              <HeaderItem
                column={column}
                index={index}
                hardWith="120px"
                setMenuOptions={function (value: React.SetStateAction<{}>): void {
                  null;
                }}
                editColumns={false}
              ...
              <div>
                ...
                {errors[column.name] && <div className="text-red-500 text-[12px]">{errors[column.name]}</div>}
              </div>
            </div>
          );
        })}
      ...
      <Tab.Group>
        <Tab.List className="flex space-x-1 mt-10  border-t border-gray-200 pt-5 ">
          {tabs.map((tab) => (
            <Tab
              ...
              className={({ selected }) =>
                `${
                  selected ? "bg-indigo-500 text-white" : "text-gray-500 hover:bg-indigo-200 hover:bg-opacity-50"
        ...
        <Tab.Panels className="mt-2 rounded-md p-1.5 border border-gray-200 bg-gray-50 min-h-[20px]">
          <Tab.Panel className="fadeIn ">
            <FormSlides publicEstablishmentId={publicEstablishmentId} row={row} />
          </Tab.Panel>
          <Tab.Panel className=" fadeIn">
            <FormArticles publicEstablishmentId={publicEstablishmentId} row={row} />
          </Tab.Panel>
          <Tab.Panel className="fadeIn">
