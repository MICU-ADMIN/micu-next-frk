...
const SketchPickerElement = dynamic(() => import("../SketchPicker/SketchPicker"), { loading: () => <Spinner /> });
const TableElement = dynamic(() => import("../TableElement/TableElement"), { loading: () => <Spinner /> });
...
const Fields = ({ field, model, onChange, errors, fields, setFocusedModels, setErrors, showFloatingLabel, submitAction }: Props) => {
  ...
  switch (field.type) {
      ...
      return (
        <>
          ...
          <input
            ...
            disabled={disabled}
            className={` ${field.subType == "color" ? "" : "base-input min-w-[140px]"}  ${errors[field.name] && "error-input"}
             ${field.className || ""}`}
    ...
    case "select":
      return (
        <SelectComponent
          ...
          onChange={(e, l, option) => {
            onChange(e, field);
            field.customChange && field.customChange(e, l, option);
          }}
          placeholder={!showFloatingLabel ? field.placeholder + (field.required ? " *" : "") : ""}
          {...field}
          name={field.name}
    ...
    case "editor":
      return (
        <div className="w-full bg-white base-input">
          <Editor
            theme="bubble" // @ts-expect-error
            value={model[field.name] || ""} // @ts-expect-error
            disableFocus={field?.disableFocus}
            onChange={(v: string) => {
              onChange(v, field);
            }}
          />
    ...
    case "color":
      return (
        <SketchPickerElement
          ...
          onChange={(e) => onChange(e.hex, field)}
          resetColor={() => onChange("", field)}
    ...
    case "checkbox":
      return (
        <div key={field.name} className={`checkbox-container ${field.className || ""}`}>
          ...
          />
          <label
            onClick={(e) => onChange(!model?.[field.name], field)}
            className="relative bottom-[2px] ml-2 cursor-pointer text-gray-800"
            htmlFor={field.name}
          >
            {field.title}
          </label>
    ...
    case "table":
      return <TableElement field={field} data={model} onChange={onChange} errors={errors} showFloatingLabel={showFloatingLabel} />;
