...
const SketchPickerElement = dynamic(
  () => import("../SketchPicker/SketchPicker"),
  { loading: () => <Spinner /> }
);
const TableElement = dynamic(() => import("../TableElement/TableElement"), {
  loading: () => <Spinner />,
});
...
const Fields = ({
  ...
  const disabled: boolean = field.disabled
    ? field.disabled({ model, field, fields })
    : false;
  ...
  switch (field.type) {
      ...
      return (
        <>
          ...
          <input
            ...
            disabled={disabled}
            className={` ${
              field.subType == "color" ? "" : "base-input min-w-[140px]"
            }
             ${field.className || ""}`}
      ...
      return (
        <select
          ...
          value={
            !field.multiple
              ? model[field.name]
              : formatSelctMultipleValue(model[field.name])
          }
          onFocus={onFocus}
          onBlur={onBlur} // @ts-expect-error
          disabled={disabled}
          className={`base-input  min-w-[140px] ${
    ...
    case "checkbox":
      return (
        <div
          key={field.name}
          className={`checkbox-container ${field.className || ""}`}
        >
          <input
            type="checkbox"
            className="rounded-md border-gray-300 text-primary-600 shadow-sm hover:border-primary-400 focus:ring focus:ring-primary-200 "
            name={field.name}
          ...
          <label
            ...
            className="relative bottom-[2px] ml-2 cursor-pointer"
            htmlFor={field.name}
          >
            {field.title}
          </label>
    ...
    case "toggle":
      return (
        <div
          key={field.name}
          className={`flex flex-row ${field.className || ""}`}
        >
