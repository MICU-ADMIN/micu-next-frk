...
function Form({ fields, model, setModel, loading, onSubmit, className, cancelLabel, submitLabel, id, onCancel = null, title, titleStyle }: Props) {
  ...
  return (
    ...
    `}
    >
      {title && <p style={titleStyle ? { ...titleStyle } : {}}>{title}</p>}
      {fields.map(
        (
          field: {
            fields: any;
