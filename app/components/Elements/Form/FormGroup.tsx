...
function FormGroup({
  ...
  const onChange = (
    ...
    !error &&
      errors[field.name] &&
      setErrors((errors: any) => ({ ...errors, [field.name]: "" }));
  };

  return (
    <div
      className={` flex mt-1 h-full ${className} ${
        horizontal ? "w-full  flex-row flex-wrap " : "flex-col"
      }  `}
    >
      {fields.map((field, i) => {
        ...
        return (
          <div
            key={field.name}
            id={field.name}
            className={`relative ${field.description ? "mb-6" : "mb-5"} flex ${
              field?.wrapperHorizontal ? "flex-row justify-between" : "flex-col"
            } ${field.wrapperClassName}`}
          >
            {field.label ? (
              <label
                className={` text-[15px] ${labelClass}`}
                htmlFor={field.name}
              >
            ...
            {(errors[field.name] || field.description) && (
              <span
                className={`slideInDown absolute -bottom-[15px] left-0 text-xs ${
                  errors[field.name]
