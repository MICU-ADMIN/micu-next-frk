  ...
  return (
    <div className={` flex mt-1 h-full ${className} ${horizontal ? "w-full  flex-row flex-wrap " : "flex-col"}  `}>
      {fields.map((field, i) => {
        ...
        const labelClass = `  ${errors[field.name] ? "text-red-500" : focusedModels[field.name] ? "text-indigo-600 text-sm" : "text-gray-800 text-sm"}   ${
          field.labelClass || ""
        }`;

        const showFloatingLabel =
          field.placeholder && (focusedModels[field?.name] || model[field?.name] || field?.type === "select" || field?.type === "nativeSelect");
