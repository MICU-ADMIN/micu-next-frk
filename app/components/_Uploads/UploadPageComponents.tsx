...
export const UploadPageComponentView = ({
  ...
  const onSetUploads = (uploads: any, upload = false) => {
    ...
    setLastIds((prev) => ({
      ...prev,
      [currentTab]: uploads[uploads.length - 1]?.id,
    }));
    ...
    if (upload) {
      setUploads(
        (prev: { images: any; videos: any; audio: any; other: any }) => ({
  ...
  const fetchUploads = async () => {
    ...
    if (currentTab === "Recently Added")
      response = await getFiles(
        options.page,
        files.length > 0 ? files[files.length - 1].id : lastIds[currentTab] || 0
      );
  ...
  const onSelectFiles = () => {
    ...
    selectionProps?.allowMultiple
      ? selectionProps.onFilesSelect(seletedFiles)
      : selectionProps.onFileSelect(seletedFiles[0]);
  };
    ...
    <>
      ...
      {selection && seletedFiles.length !== 0 && (
        <Button
          ...
          className="absolute top-3 right-[130px] z-50"
        >
          Confirm Selection
        </Button>
      )}
                      ...
                      <div
                        ...
                        onClick={() => {
                          ...
                          if (selection && upload.public) {
                            selectionProps?.allowMultiple
                              ? setSelectedFiles((prev) => [...prev, upload])
                              : setSelectedFiles([upload]);
                            selectionProps?.allowMultiple
                              ? setSelectedIds((prev) => [...prev, upload.id])
                              : setSelectedIds([upload.id]);
                          } else if (selection && !upload.public) {
                            errorMessage(
                              "Only public files can be added to the page. Please make the file public and try again."
                            );
                          }
                        }}
                      >
                        <UploadPageEachLayout
                          name={upload.originalname}
                          key={upload.key}
                          source={encodeURI(pUrl + upload.key)}
                          current={false}
                          file={upload}
                        />
      ...
      {uploading && (
        <div
          style={{ zIndex: 999999 }}
          className="slideInLeft fixed left-[50px] top-5 rounded-lg  bg-white text-primary-500
...
export function UploadPageEachLayout({
  current,
  name,
  file,
  key,
  source,
}: UploadPageCellEachProps) {
  return (
    <li key={key} className="relative">
      ...
      >
        {file.cat === "image" ? (
          ...
          >
            <DocumentIcon
              className=" m-auto h-full text-gray-300 "
              aria-hidden="true"
            />
          </div>
        )}

        <p className=" absolute bottom-[10px] left-0 flex items-center justify-center rounded bg-primary-200 p-1 text-sm font-medium text-primary-500">
