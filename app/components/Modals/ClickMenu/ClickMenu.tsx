    ...
    <div
      ...
      style={
        position
          ? {
              top:
                window.innerHeight - position.y > 224
                  ? position.y
                  : position.y - 224,
              left:
                window.innerWidth - position.x > 224
                  ? position.x
                  : position.x - 225,
              position: "fixed",
              zIndex: 99999,
      ...
      className=" scaleIn items-center overflow-hidden overflow-hidden rounded-md border border-gray-200  p-2 drop-shadow-lg "
    >
      <input
      ...
      {filteredItems().map((item: any) => {
        ...
        return (
          <div
            ...
            }  group-data-[disabled]:text-mauve8 hover-text-in group pointer-events-auto relative mt-1 flex h-[25px] cursor-pointer select-none items-center rounded-[3px] bg-transparent px-[5px] pl-[12px] text-[13px] font-medium leading-none outline-none backdrop-blur-3xl hover:text-primary-500 hover:opacity-90
            `}
          >
            {(item.type === "delete" || item.icon) && (
              ...
              >
                {item.type === "delete" ? (
                  <Delete className="w-[14px] text-red-500" />
                ) : (
                  item.icon
                )}
              </div>
      ...
      <div
        ...
        className="text-gray-500 mt-3 text-xs leading-none"
      />
    </div>
  );
}
