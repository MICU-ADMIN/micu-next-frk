...
import {
  ...
  removeFromCache,
  requestHandler,
} from "@/app/_helpers/web/requestHandler";
...
function Nav({
  ...
  const getUser = async () => {
    const u = await getCacheData("currentUser", true);
    setUser(u?.User || u);
  };
  async function userNavigationFunctionHandler(func: string | undefined) {
    switch (func) {
      ...
      case "prayerscreens":
        router.push(
          "/dashboard/prayerscreens/" + publicId(currentEstablishment)
        );
        break;
      case "articles":
  ...
  return (
    <>
      {setNavOpen && (
        <MenuIcon
          className={`h-6 w-5 fixed right-2 top-2  cursor-pointer hover:bg-primary-500 hover:bg-opacity-10   focus:outline-none focus:ring-2 focus:ring-white rounded-md hover:shadow-lg  ${
            !navOpen ? "text-primary-500" : "text-white"
          }`}
      ...
      {navOpen ? (
        <div
          ...
          className="bg-gradient-to-r from-primary-700 to-primary-500 pb-14 transition duration-500 ease-in-out"
        >
          <>
                ...
                <div className="hidden lg:ml-4 lg:flex lg:items-center lg:py-5 lg:pr-0.5">
                  {currentEstablishment && orgUsers && orgUsers.length > 0 && (
                    <div className=" absolute right-[100px] top-[8px] mr-5 flex w-[150px]  flex-shrink-0 flex-col  ">
                      ...
                      <select className="block  rounded-md border-0 p-1 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-1 focus:ring-inset focus:ring-primary-500 sm:text-sm sm:leading-6">
                  ...
                  >
                    <div>
                      <div className="flex rounded-full bg-white text-sm ring-2 ring-white ring-opacity-20 focus:outline-none focus:ring-opacity-100">
                        ...
                        <div className="flex h-8 w-8 items-center justify-center rounded-full  text-lg font-medium text-primary-800">
                      ...
                      <nav className="mt-1 flex space-x-4">
                        {divItems.map((item) => (
                          <div
                            key={item.name}
                            onClick={() =>
                              userNavigationFunctionHandler(item.name)
                            }
                      ...
                      <div className="divide-y divide-gray-200 rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
                        <div className="pb-2 pt-3">
                          <div className="flex items-center justify-between px-4">
                            <div>
                              {/* <img className="h-8 w-auto" src={require("src/assets/logo.png")} alt="Your Company" /> */}
                            </div>
                            <div className="-mr-2">
                              <button
                                onClick={() =>
        ...
        <div className="absolute top-0 left-2 flex-shrink-0   w-fit h-[35px] rounded  transition duration-500 ease-in-out">
          <Link
            ...
            href={"/dashboard/home/" + publicId(currentEstablishment)}
          >
            {/* <span className="sr-only">Your Company</span> */}
            {/* <img className="h-8 w-auto" src={require("src/assets/logo.png")} alt="" /> */}
            <Image
