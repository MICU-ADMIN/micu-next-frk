...
const publicId = (currentEstablishment: { establishmentName: any; name: any }) => {
  ...
  async function userNavigationFunctionHandler(func: string) {
    switch (func) {
      ...
      case "home":
        ...
        break;
      case "prayerscreens":
        router.push("/dashboard/prayerscreens/" + publicId(props.currentEstablishment));
        break;
      case "articles":
        router.push("/dashboard/articles/" + publicId(props.currentEstablishment));
        break;
      case "upload":
        router.push("/dashboard/upload/" + publicId(props.currentEstablishment));
        break;
      case "sites":
        router.push("/dashboard/sites/" + publicId(props.currentEstablishment));
        break;
      case "prayers":
        router.push("/dashboard/prayers/" + publicId(props.currentEstablishment));
        break;
      case "records":
        router.push("/dashboard/records/" + publicId(props.currentEstablishment));
        break;
      case "events":
        router.push("/dashboard/events/" + publicId(props.currentEstablishment));
        break;
      case "analytics":
        router.push("/dashboard/analytics/" + publicId(props.currentEstablishment));
        break;
      case "settings":
        router.push("/dashboard/settings/" + publicId(props.currentEstablishment));
        break;
      case "reports":
        router.push("/dashboard/reports/" + publicId(props.currentEstablishment));
        break;
      case "assets":
        router.push("/dashboard/assets/" + publicId(props.currentEstablishment));
        break;
      case "scheduling":
        router.push("/dashboard/scheduling/" + publicId(props.currentEstablishment));
        break;
      case "help":
    ...
    <>
      <div>
          ...
          {showMobileMenu ? (
            <div className="relative z-50 lg:hidden slideLeft">
              <div
                ...
                leaveTo="opacity-0"
              >
                <div className="fixed inset-0 bg-gray-900/80" onClick={() => setShowMobileMenu(false)}></div>
              </div>
                            ...
                            >
                              {props.sideBar ? (
                                ...
                                <ul className="-mx-2 space-y-1 h-screen">
                                  {navigation.map((item) => (
                                    <li key={item.name} onClick={() => userNavigationFunctionHandler(item.href)}>
                                      <p className="group text-3xl transition duration-300 hover:text-indigo-600 cursor-pointer mb-2">{item.name}</p>
                                    </li>
                                  ))}
                                </ul>
                              )}
          ...
          <div className="flex grow flex-col gap-y-5 overflow-hidden  border-r border-gray-200 bg-[#F8F9FA] px-6">
            ...
            <nav className="flex flex-1 flex-col">
              <ul className="flex flex-1 flex-col gap-y-7  pt-10">
                      ...
                      <>
                        <ul className="mx-2">
                          {navigation.map((item) => (
                            <div key={item}>
                              <button
                                onClick={() => userNavigationFunctionHandler(item.href)}
                                style={{
                                  whiteSpace: "nowrap",
                                }}
                                className="group cursor-pointer text-2xl text-black subpixel-antialiased transition duration-300 hover:text-indigo-600"
                              >
                                {item.name}
                ...
                <li
                  onClick={() => {
                    router.push("/dashboard/account/" + publicId(props.currentEstablishment));
                  }}
        ...
        <div className="sticky top-0 z-40 flex items-center gap-x-6 bg-[#F8F9FA] px-4 py-4 shadow-sm sm:px-6 lg:hidden">
          <button type="button" className="-m-2.5 p-2.5 text-gray-700 lg:hidden" onClick={() => setShowMobileMenu(true)}>
...
function SidebarFadeAnimation(props: {
  ...
  return (
    <div>
      {props.sideBar ? (
        <button className="mb-10 flex align-center" onClick={() => window.history.back()}>
          <ChevronLeft /> Go back
        </button>
      ) : null}
      <div
