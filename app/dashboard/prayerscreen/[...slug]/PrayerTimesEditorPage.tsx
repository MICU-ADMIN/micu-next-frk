...
import PrayertimesScreen from "@/app/components/_PrayerTimes/PrayerTimesEditor/PrayerTimesScreen";
...
`);
  ...
  React.useEffect(() => {
    if (prayerTimes && prayerTimes.length > 0) {
      ...
      // )
      const curPrayerTimeIndex = prayerTimes.findIndex(
        (p) => p.date === curDate
      );

      setCurIndex(
        curPrayerTimeIndex && curPrayerTimeIndex > -1 ? curPrayerTimeIndex : 0
      );
  ...
  return (
    <>
      <div className="h-full w-full">
        <div className=" position-sticky top-0  z-10 flex h-[40px] w-full flex-row justify-between border-b border-gray-300 bg-gray-100 shadow-sm">
          ...
          <div className="mr-2 flex items-center">
            <button
              ...
              className="0 mr-2 rounded-md border  border-transparent px-2 py-1 text-sm text-primary-800 hover:bg-primary-200 focus:outline-none"
        ...
        <div
          className={styles.prayerTimesWrapper}
          style={{ fontFamily: theme?.fontFamily + "!important" }}
        >
