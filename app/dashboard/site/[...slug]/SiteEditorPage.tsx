...
const SectionMenu = dynamic(
  () => import("@/app/components/_Sites/SiteEditor/sections/SectionMenu"),
  { ssr: false }
);
...
function SiteEditorPage({ currentSite, isPublic = false }) {
  ...
  React.useEffect(() => {
    ...
    if (currentSite) {
      ...
      requestHandler({
        type: "get",
        route: `sites/single?siteName=` + name,
        shouldCache: true,
        returnCache: true,
      }).then((res) => {
        setTimeout(() => {
    ...
    if (currentPage?.content && !diableEditStates) {
      ...
      undoTimout.current = setTimeout(() => {
        if (
          ...
          JSON.stringify(editStates[editStates.length - 1])
        )
          return;
  ...
  const setSitePage = async (site: any) => {
    ...
    if (typeof site.SitePages[pageIndex].content === "string") {
      ...
      // )
      site.SitePages[pageIndex].content = await JSON.parse(
        site.SitePages[pageIndex].content
      );
    }
  ...
  const save = async () => {
    ...
    requestHandler({
      type: "put",
      route: "sitePage",
      body: { id: currentPage.id, content: json, siteName: siteName },
    }).then((res) => {
      setLoading(false);
      if (!res?.errors) {
  ...
  const undo = (index: number) => {
    console.log("undo", index, editStates);
        ...
        >
          ...
          {currentPage && currentPage?.content?.length > 0
            ? currentPage.content.map(
                (
                  page: { id: React.Key | null | undefined; data: any },
                  i: React.SetStateAction<number> | undefined
                ) => {
                  return (
          ...
          {!readOnly && (
            <p
              ...
              className="mt-1 cursor-pointer rounded px-2  py-1 text-center text-primary-500 hover:bg-primary-100 hover:text-primary-700"
            >
              Add Section
            </p>
          )}
