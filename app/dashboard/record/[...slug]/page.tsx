...
async function getData(id: string) {
  try {
    ...
    const [recordData] = await (
      ...
      " SELECT `id`, `recordId`, `model`, `createdAt`, `userId`, `uploadIds`, `index`, `createdAt`, `updatedAt` FROM `RecordData` WHERE `recordId` = ? AND `establishmentId` = ? ORDER BY `id` ASC  LIMIT 30 ",
